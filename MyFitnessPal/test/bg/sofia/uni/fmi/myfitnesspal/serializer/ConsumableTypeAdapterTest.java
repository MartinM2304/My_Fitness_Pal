...
class ConsumableTypeAdapterTest {
    ...
    void testSerialize_Food() {
        ...
        when(serializationContextMock.serialize(consumptionLog)).thenReturn(new JsonPrimitive("mocked-log"));
        ...
        assertEquals("mocked-log", jsonObject.get("consumptionLog").getAsString());
    }

    @Test
    void testSerialize_Water() {
    ...
    @Test
    void testSerialize_ReflectionFailure() {
        Food food = new Food.Builder("Apple").build();
        when(serializationContextMock.serialize(any())).thenThrow(new RuntimeException("Serialization error"));
    ...
    void testDeserialize_FoodWithConsumptionLog() {
        ...
        jsonObject.addProperty("name", "Banana (120g; 89 kcal; 23.0g, 0.3g, 1.10g)");
    ...
    void testDeserialize_FoodNoOptionalFields() {
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("type", "Food");
        jsonObject.addProperty("name", "Orange (130g; 47 kcal; 12.0g, 0.1g, 0.90g)");

        Consumable result = adapter.deserialize(jsonObject, Consumable.class, deserializationContextMock);

        assertTrue(result instanceof Food);
        Food food = (Food) result;
        assertEquals("Orange (130g; 47 kcal; 12.0g, 0.1g, 0.90g)", food.toString());
    }
}